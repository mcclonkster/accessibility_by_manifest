...
- Confidence: high
- Notes: `accessibility_by_manifest/` is now best understood as the broad shared core for inputs, evidence extraction, normalization, review signals, and shared projection helpers. `src/pdf_accessibility/` is the first fully integrated shared-state v0.1.0 PDF workflow vertical with finalization gating.
...
The safest package relationship is now:

- `accessibility_by_manifest/` as the broad evidence/extraction/normalization/projection layer
- `src/pdf_accessibility/` as the narrow v0.1.0 workflow/remediation layer
- integration by deliberate reuse and interfaces later, not by immediate package collapse

## Conflicts Or Disagreements

- `README.md`, `PROJECT.md`, and `AGENTS.md` now present a layered repo story rather than a split one: shared core in `accessibility_by_manifest/`, governed PDF workflow vertical in `src/pdf_accessibility/`, and HTML as a sibling output path from the same normalized model.
