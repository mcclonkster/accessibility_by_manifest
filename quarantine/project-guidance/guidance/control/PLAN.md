...
The repo should converge toward this broader architecture:
...
- `accessibility_by_manifest/` should remain the broad input/extraction/normalization/projection core.
- `src/pdf_accessibility/` should remain the first active format-specific workflow vertical.
- PDF is the current active vertical, not the whole architecture.
- Accessible HTML should be treated as a sibling output workflow beside accessible PDF, not as a PDF-specific side effect.
- Future workflows for other targets should consume the shared normalized model rather than rebuilding format-specific extraction and normalization stacks.
...
- Keep the broader architecture wide enough for many inputs and many output-specific workflows even while PDF is the first active vertical.
...
- Do not let the architecture collapse into “PDF-only everywhere” just because PDF is the current active vertical.
...
- Outcome: The relationship between `accessibility_by_manifest/` and `src/pdf_accessibility/` is explicit and guides future edits.
- Acceptance criteria:
  - One documented decision says the repo uses a layered relationship.
...
- Expected checkpoint output: updated decision record and doc edits that explain which work belongs in which package layer.
...
- What should be true: the package-relationship decision and generated-output cleanup policy should be explicit enough that README/project-wide edits can proceed safely.
