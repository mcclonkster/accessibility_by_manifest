...
def plan_runs(pptx_files: list[Path], input_path: Path, output_root: Path | None) -> list[DeckRun]:
    ...
    for pptx_path in pptx_files:
        if output_root is None:
            output_dir = pptx_path.parent / f"{pptx_path.stem}_manifest_output"
        elif batch_mode:
            output_dir = output_root / f"{safe_name(pptx_path.stem)}_manifest_output"
        else:
            output_dir = output_root

        if output_dir in used_dirs:
            output_dir = output_dir.parent / f"{safe_name(pptx_path.stem)}_{safe_name(pptx_path.parent.name)}_manifest_output"
...
def output_paths(run: DeckRun) -> OutputPaths:
    ...
    return OutputPaths(
        ...
        review_report=run.output_dir / f"{stem}_review_report.md",
