...
- **Zero runtime risk** — Pure Markdown skill. The entire repository contains only `.md` files — no hooks, no shell scripts, no executable code, no `package.json`, no build step. Nothing runs on install or invocation beyond the active agent harness loading the Markdown skill.

## Installation

This skill ships with the ECC bundle. No separate installation is needed when ECC is installed.
...
If you are vendoring only this skill outside the full ECC install, copy the reviewed file from the ECC repository into the skill directory used by your agent harness, such as `.agents/skills/blueprint/SKILL.md` or `~/.claude/skills/blueprint/SKILL.md`. Vendored copies do not have a git remote, so update them by re-copying the file from a reviewed ECC commit rather than running `git pull`.

## Requirements

- A coding agent harness such as Codex, Claude Code, or OpenCode. A `/blueprint` slash command is optional; invoking the skill by name is sufficient when slash commands are not available.
