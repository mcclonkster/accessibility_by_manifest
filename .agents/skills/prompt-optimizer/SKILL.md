...
- User says "what's the best way to ask Codex, Claude Code, OpenCode, or another coding agent to..."
...
1. Check if an agent instruction file such as `AGENTS.md`, `CLAUDE.md`, or `.agents/**` exists in the working directory — read it for project conventions
...
|-------|------------------|-----------|
| TRIVIAL-LOW | Default coding model | Fast, cost-efficient for simple tasks |
| MEDIUM | Default coding model | Best balance for standard work |
| HIGH | Default coding model (main) + strongest reasoning model (planning) | Strongest model for architecture, default model for implementation |
| EPIC | Strongest reasoning model (blueprint) + default coding model (execution) | Deep reasoning for multi-session planning |
...
| Model | Default coding model | Recommended for this scope |
...
Recommended: strongest reasoning model for blueprint planning, default coding model for phase execution.
```
