...
# Supported harnesses pass hook data via stdin as JSON. The parser accepts the
# Claude Code hook schema and compatible schemas from other harnesses.
#
...
# Can also be registered manually in the active harness settings file.
...
# Read JSON from stdin (harness hook format)
INPUT_JSON=$(cat)
...
if [ -n "$STDIN_CWD" ] && [ -d "$STDIN_CWD" ]; then
  ...
  export CLV2_PROJECT_DIR="${_GIT_ROOT:-$STDIN_CWD}"
  export CLAUDE_PROJECT_DIR="${CLV2_PROJECT_DIR}"
fi
...
CONFIG_DIR="${CLV2_HOMUNCULUS_DIR:-${HOME}/.agents/homunculus}"
if [ ! -d "$CONFIG_DIR" ] && [ -d "${HOME}/.claude/homunculus" ]; then
...
case "${AGENT_ENTRYPOINT:-${CLAUDE_CODE_ENTRYPOINT:-cli}}" in
...
try:
    ...
    # Some harnesses do not include a "hook_type" field in the stdin JSON,
    # so we rely on the shell argument ("pre" or "post") instead.
    hook_phase = os.environ.get("HOOK_PHASE", "post")
    event = "tool_start" if hook_phase == "pre" else "tool_complete"

    # Extract fields - harness hook format
...
# which caused runaway parallel observer analysis processes.
