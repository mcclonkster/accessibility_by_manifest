...
#
# Detection priority:
#   1. CLV2_PROJECT_DIR or harness-specific project env var (if set)
...
elif [ -d "${HOME}/.agents/homunculus" ]; then
  _CLV2_HOMUNCULUS_DIR="${HOME}/.agents/homunculus"
elif [ -d "${HOME}/.claude/homunculus" ]; then
...
_clv2_detect_project() {
  ...
  # 1. Try explicit project env vars
  local env_project_dir="${CLV2_PROJECT_DIR:-${CLAUDE_PROJECT_DIR:-}}"
  if [ -n "$env_project_dir" ] && [ -d "$env_project_dir" ]; then
    project_root="$env_project_dir"
    source_hint="env"
  fi
