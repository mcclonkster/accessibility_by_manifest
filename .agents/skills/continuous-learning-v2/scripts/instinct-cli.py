...
_ENV_HOMUNCULUS_DIR = os.environ.get("CLV2_HOMUNCULUS_DIR")
if _ENV_HOMUNCULUS_DIR:
    HOMUNCULUS_DIR = Path(_ENV_HOMUNCULUS_DIR).expanduser()
elif (Path.home() / ".agents" / "homunculus").exists():
    HOMUNCULUS_DIR = Path.home() / ".agents" / "homunculus"
elif (Path.home() / ".claude" / "homunculus").exists():
...
def detect_project() -> dict:
    ...
    # 1. Explicit project env vars
    env_dir = os.environ.get("CLV2_PROJECT_DIR") or os.environ.get("CLAUDE_PROJECT_DIR")
...
def cmd_projects(args) -> int:
    ...
    if not registry:
        ...
        print("Projects are auto-detected when you use a supported coding agent in a git repo.")
        return 0
