...
# parallel observer analysis processes.
...
analyze_observations() {
  ...
  if [ "${CLV2_IS_WINDOWS:-false}" = "true" ] && [ "${ECC_OBSERVER_ALLOW_WINDOWS:-false}" != "true" ]; then
    echo "[$(date)] Skipping observer analysis on Windows due to known non-interactive CLI hang risk (#295). Set ECC_OBSERVER_ALLOW_WINDOWS=true to override." >> "$LOG_FILE"
    return
  fi

  OBSERVER_AGENT_CLI="${CLV2_OBSERVER_AGENT_CLI:-claude}"
  if ! command -v "$OBSERVER_AGENT_CLI" >/dev/null 2>&1; then
    echo "[$(date)] Observer agent CLI not found ($OBSERVER_AGENT_CLI), skipping analysis" >> "$LOG_FILE"
    return
  fi
  ...
  # prefixes (e.g. /c/Users/...) that some agent subprocesses cannot resolve.
...
PROMPT
  ...
  # resolution, so relying on cat "$prompt_file" inside the agent invocation
  ...
  ECC_SKIP_OBSERVE=1 ECC_HOOK_PROFILE=minimal "$OBSERVER_AGENT_CLI" --model haiku --max-turns "$max_turns" --print \
  ...
  if [ "$exit_code" -ne 0 ]; then
    echo "[$(date)] Observer analysis failed (exit $exit_code)" >> "$LOG_FILE"
  fi
