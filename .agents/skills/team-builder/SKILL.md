...
Agents are discovered via harness-native commands and file globs, merged and deduplicated by agent name:

1. **Harness agent list command** (preferred when available) — use the active CLI's agent listing command to get all known agents, including user agents, plugin agents, and built-in agents. For Claude Code this is `claude agents`; for Codex/OpenCode, use the equivalent command if the CLI exposes one.
2. **File glob** (fallback, and for reading agent content) — agent markdown files are read from:
   - `./agents/**/*.md` + `./agents/*.md` — project-local agents
   - `.agents/agents/**/*.md` + `.agents/agents/*.md` — project-local shared agents
   - harness-specific global paths such as `~/.claude/agents/**/*.md` + `~/.claude/agents/*.md`
...
Run the active harness's agent listing command, if available, to get the full agent list. Parse each line:
- **Plugin agents** may be prefixed with `plugin-name:` (e.g., `ecc:security-reviewer`). Use the part after `:` as the agent name and the plugin name as the domain.
- **User agents** may have no prefix. Read the corresponding markdown file from `.agents/agents/`, `./agents/`, or the harness-specific global agent path to extract the name and description.
...
If no agents are found after probing the harness and file locations, inform the user: "No agents found. Verify your agent directories or the active harness's agent listing command." Then stop.
...
- **Parallel agent calls, not group chat by default.** This skill uses parallel harness-native subagent calls for independent work. A multi-agent dialogue tool is only needed when agents must debate or respond to each other.
