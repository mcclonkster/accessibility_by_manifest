...
Safely open-source any project through a 3-stage pipeline: **Fork** (strip secrets) → **Sanitize** (verify clean) → **Package** (AGENTS.md or harness-specific instructions + setup.sh + README).
...
| `/opensource package PROJECT` | Generate AGENTS.md or harness-specific instructions + setup.sh + README |
...
Generate:
1. Agent instruction file such as AGENTS.md or CLAUDE.md (commands, architecture, key files)
...
License: {license}
Files generated:
  - AGENTS.md or harness-specific instruction file
  - setup.sh (executable)
  - README.md
  - LICENSE
  - CONTRIBUTING.md
...
Show each project with pipeline progress (FORK_REPORT.md, SANITIZATION_REPORT.md, and agent instruction file presence).
...
$HOME/opensource-staging/
  my-project/
    ...
    AGENTS.md                # From packager agent by default
