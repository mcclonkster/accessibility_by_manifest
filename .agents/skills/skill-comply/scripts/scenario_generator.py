...
from scripts.utils import agent_cli, extract_yaml
...
def generate_scenarios(
    ...
    Calls the configured agent CLI with the scenario_generator prompt, parses YAML output.
    """
    ...
    if result.returncode != 0:
        raise RuntimeError(f"{cli} -p failed: {result.stderr}")

    if not result.stdout.strip():
        raise RuntimeError(f"{cli} -p returned empty output")
